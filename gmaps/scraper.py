...
class GoogleMapsEngine:
    ...
    def search_query(self, query_or_url: str) -> bool:
        ...
        # Bypass Google Consent banner if shown
        try:
            for btn_xpath in ["//button[contains(@aria-label, 'Accept')]", "//button[contains(., 'Accept all')]", "//form//button"]:
    ...
    def scroll_results_pane(self) -> None:
        try:
            feed = self.driver.find_element(By.XPATH, "//div[@role='feed'] | //div[contains(@aria-label, 'Results for')]")
    ...
    def extract_visible_cards(self) -> List:
        # Matches all Google Maps result anchor links
        return self.driver.find_elements(By.XPATH, "//div[@role='feed']//a[contains(@href, '/maps/place/')] | //a[contains(@href, '/maps/place/')]")
    ...
    def parse_card_details(self, card) -> Optional[Dict[str, str]]:
        try:
            ...
            # Extract phone
            try:
                phone_btn = self.driver.find_element(By.XPATH, "//button[starts-with(@data-item-id, 'phone:')] | //button[contains(@aria-label, 'Phone')]")
            ...
            # Extract address
            try:
                addr_btn = self.driver.find_element(By.XPATH, "//button[@data-item-id='address'] | //button[contains(@aria-label, 'Address')]")
            ...
            # Extract website
            try:
                web_btn = self.driver.find_element(By.XPATH, "//a[@data-item-id='authority'] | //a[contains(@aria-label, 'Website')]")
            ...
            # Extract category
            try:
                cat_btn = self.driver.find_element(By.XPATH, "//button[contains(@jsaction, 'pane.rating.category')]")
